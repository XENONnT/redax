...
V1724::~V1724(){
  ...
  LOG(MongoLog::Local, "%s", msg.str().c_str());
}
...
int V1724::Init(int link, int crate) {
  ...
  if(a != cvSuccess){
    LOG(MongoLog::Warning, "Board %i failed to init, error %i handle %i link %i bdnum %i",
            fBID, a, fBoardHandle, link, crate);
    fBoardHandle = -1;
    return -1;
  }
  LOG(MongoLog::Debug, "Board %i initialized with handle %i (link/crate)(%i/%i)",
  ...
  if (Reset()) {
    LOG(MongoLog::Error, "Board %i unable to pre-load registers", fBID);
    return -1;
  } else {
    LOG(MongoLog::Local, "Board %i reset", fBID);
  }
  ...
  if (fOptions->GetInt("do_sn_check", 0) != 0) {
    if ((word = ReadRegister(fSNRegisterLSB)) == 0xFFFFFFFF) {
      LOG(MongoLog::Error, "Board %i couldn't read its SN lsb", fBID);
      return -1;
    }
    my_bid |= word&0xFF;
    if ((word = ReadRegister(fSNRegisterMSB)) == 0xFFFFFFFF) {
      LOG(MongoLog::Error, "Board %i couldn't read its SN msb", fBID);
      return -1;
    }
    my_bid |= ((word&0xFF)<<8);
    if (my_bid != fBID) {
      LOG(MongoLog::Local, "Link %i crate %i should be SN %i but is actually %i",
...
std::tuple<uint32_t, long> V1724::GetClockInfo(std::u32string_view sv) {
  ...
  LOG(MongoLog::Message, "No clock info for %i?", fBID);
  return {0xFFFFFFFF, -1};
}
...
int V1724::GetClockCounter(uint32_t timestamp){
  ...
  if (n_missed > 0) {
    LOG(MongoLog::Message, "Board %i missed %i rollovers", fBID, n_missed);
  ...
  if (timestamp < fLastClock) {
    ...
    fRolloverCounter++;
    LOG(MongoLog::Local, "Board %i rollover %i (%x/%x)",
...
int V1724::WriteRegister(unsigned int reg, uint32_t value){
  ...
  if((ret = CAENVME_WriteCycle(fBoardHandle, fBaseAddress+reg,
    ...
    LOG(MongoLog::Warning,
		"Board %i write returned %i (ret), reg 0x%04x, value 0x%08x",
		fBID, ret, reg, value);
    return -1;
  }
  if (echo) LOG(MongoLog::Local, "Board %i wrote 0x%x to 0x%04x", fBID, value, reg);
...
unsigned int V1724::ReadRegister(unsigned int reg){
  ...
  if((ret = CAENVME_ReadCycle(fBoardHandle, fBaseAddress+reg, &temp, cvA32_U_DATA, cvD32)) != cvSuccess){
    LOG(MongoLog::Warning, "Board %i read returned: %i (ret) 0x%x (val) for reg 0x%04x", fBID, ret, temp, reg);
    return 0xFFFFFFFF;
  }
  return temp;
}
...
int V1724::Read(std::unique_ptr<data_packet>& outptr){
  ...
  if (status & 0x10) {
    ...
    LOG(MongoLog::Local, "%s", msg.c_str());
  }
  // Initialize
  ...
  do{
    ...
    total_bytes += nb;
    if (total_bytes > fBufferSize) LOG(MongoLog::Debug,
...
int V1724::LoadDAC(std::vector<uint16_t>& dac_values){
  ...
  for(unsigned ch=0; ch<fNChannels; ch++){
    if ((ReadRegister(fChStatusRegister + 0x100*ch) & 0x4) || WriteRegister(fChDACRegister + 0x100*ch, dac_values[ch])){
      LOG(MongoLog::Error, "Board %i ch %i failed to set DAC (0x%x)", fBID, ch, dac_values[ch]);
      return -1;
    }
  }
  return 0;
...
bool V1724::MonitorRegister(uint32_t reg, uint32_t mask, int ntries, int sleep, uint32_t val){
  ...
  LOG(MongoLog::Warning,"Board %i MonitorRegister failed for 0x%04x with mask 0x%04x and register value 0x%04x, wanted 0x%04x",
...
int V1724::BaselineStep(std::vector<uint16_t>& dac_values, std::vector<int>& channel_finished, std::vector<double>& bl_per_channel, int step) {
  ...
  if (!EnsureReady(1000, 1000)) {
    LOG(MongoLog::Warning, "Board %i not ready for baselines", fBID);
    return -1;
  }
  SoftwareStart();
  std::this_thread::sleep_for(std::chrono::milliseconds(1));
  if (!EnsureStarted(1000, 1000)) {
    LOG(MongoLog::Warning, "Board %i can't start baselines", fBID);
    return -1;
  }
  ...
  if (!EnsureStopped(1000, 1000)) {
    LOG(MongoLog::Warning, "Board %i won't stop", fBID);
    return -1;
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(1));
  std::unique_ptr<data_packet> dp;
  if ((words_read = Read(dp)) <= 0) {
    LOG(MongoLog::Warning, "Board %i readout error", fBID);
    return -2;
  }
  if (words_read <= 4) {
    LOG(MongoLog::Local, "Board %i missing data?? %i", fBID, words_read);
    return 1;
  }
  ...
  for (unsigned ch = 0; ch < fNChannels; ch++) {
    ...
    if (counts_total == 0) {
      LOG(MongoLog::Local, "%i.%i.%i: no samples (%x)", fBID, ch, step, dac_values[ch]);
      ...
      LOG(MongoLog::Local, "%i.%i.%i: %i out of %i counts around max %li",
    ...
    if (off_by != off_by) { // dirty nan check
      ...
      LOG(MongoLog::Debug, "%i.%i.%i: NaN alert (%x)",
      ...
      if (channel_finished[ch] == convergence)
        LOG(MongoLog::Local, "%i.%i.%i converged: %.1f | %x", fBID, ch, step, baseline, dac_values[ch]);
    } else {
      ...
      LOG(MongoLog::Local, "%i.%i.%i adjust %i to %x (%.1f)", fBID, ch, step, adjustment, dac_values[ch], baseline);
    }
