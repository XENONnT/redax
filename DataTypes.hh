...
// Forward declaration so shared_ptr knows V1724 exists
// (We don't need to include the whole V1724 header here)
class V1724;

// Your original data_packet struct
struct data_packet{
...
// The new struct for passing data between threads safely
struct DataBatch {
    std::list<std::unique_ptr<data_packet>> packets;
    int bytes;
};
