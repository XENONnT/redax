...
int V1495_TPC::Arm(std::map<std::string, int>& opts) {
  ...
  unsigned int is_busy_he_used = 1;  // Firmware allows to turn this off, but we don't. Patchable if needed (here and DAQ config)
  ...
  // 1. Anti-Veto start/stop on, if HEV is not used (they are on the same output; Anti-Veto can be reconstructed)
  ...
  // 2. Anti Veto Time conversion, duration > 0
  if (is_anti_veto_active == 1) {
    if (fAntiVetoDuration_clk == 0) {
      fLog->Entry(MongoLog::Message, "V1495: Neutron Generator anti-veto duration of zero is not allowed.");
      return 1;
    }
  ...
  // 3. Fractional Lifetime mode: no HEV at the same time (they are on the same output)
  if (is_frac_lt_mode_on == 1){
      if (is_hev_on == 1) {
          fLog->Entry(MongoLog::Message, "V1495: HEV and Fractional Lifetime both on. This is not allowed!");
          return 1;
        }
  ...
  // 4. Fractional Lifetime durations conversion, both > 0
  if (is_frac_lt_mode_on == 1) {
    if (fFracLTVetoOn_clk * fFracLTVetoOff_clk == 0) {
      fLog->Entry(MongoLog::Message, "V1495: Fractionla lifetie on and off peridos cannot be 0, but on=%i, off=%i",
    ...
    fLog->Entry(MongoLog::Local, "V1495 fractional mode active: on %i, off %i (CLK cycles)",
            opts["fractional_lifetime_veto_on_us"], opts["fractional_lifetime_veto_off_us"]);
  }
